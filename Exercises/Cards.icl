...
sort_by_value cards	= sort_by_value` (sortByValue cards) (length cards - 1)
	where
		...
		| not ((cards !! (length cards - 1 - int)).value == (cards !! (length cards - 1 - int + 1)).value)	= sortBySuit (cards % (0,length cards - 1 - int)) ++ sort_by_value` (cards % (length cards - 1 - int + 1, length cards - 1)) (length (cards % (length cards - 1 - int + 1, length cards - 1)) - 1)
		| otherwise																							= sort_by_value` cards (int - 1)
...
sort_by_suit cards	= sort_by_suit` (sortBySuit cards) (length cards - 1)
	where
		...
		| not ((cards !! (length cards - 1 - int)).suit == (cards !! (length cards - 1 - int + 1)).suit)	= sortByValue (cards % (0,length cards - 1 - int)) ++ sort_by_suit` (cards % (length cards - 1 - int + 1, length cards - 1)) (length (cards % (length cards - 1 - int + 1, length cards - 1)) - 1)
		| otherwise
																									= sort_by_suit` cards (int - 1)
...
Start = sort_by_value (carddeck ++ [{value = Three, suit = Diamond}, {value = Ace, suit = Heart}])
